                            ...
                            <div class="row">
                                ...
                                <div class="form-group col-12 col-sm-6 col-md-4 col-lg-4 col-xl-2">
                                    ...
                                    <select style="width: 100%" id="parcelStatus" name="parcel_status"
                                        ...
                                        <option value="" selected> {{ __('menus.select') }} {{ __('levels.status') }}
                                        </option>
                            ...
                            <form action="{{route('parcel.specific.search') }}" method="get">
                                ...
                                <div class="d-flex parcelsearchFlex ml-0">
                                    <input id="Psearch" class="parcelml-0 form-control  group-input w-100 " name="search"
                                        type="text" placeholder="{{ __('levels.search') }}..."
                                        value="{{ $request->search }}">
                                ...
                                <div class="text-right d-flex justify-content-end parcel-index-bulk">
                                    ...
                                    <form action="{{ route('parcel.multiple.print-label') }}" method="get" target="_blank"
                                        ...
                                        <button type="submit" class="btn btn-sm btn-primary mr-2 multiplelabelprint"
                                            data-parcels='' style="display: none">{{ __('levels.print') }}</button>
                                    </form>
                                    ...
                                    @if($request->parcel_status == \App\Enums\ParcelStatus::DELIVERY_MAN_ASSIGN)
                                        <a href="{{route('parcel.parcel-bulkassign-print', ['parcels' => $parcelsPrint->pluck('id')->toArray(), 'delivery_man_id' => $request->parcel_deliveryman_id])}}"
                                            class="btn btn-sm btn-primary ml-1 mb-1" target="_blank" data-toggle="tooltip"
                                            data-placement="top" title="Print">{{ __('parcel.print') }}</a>
                                    @endif
                                    ...
                                    <a href="{{route('parcel.create')}}" class="btn btn-sm btn-primary ml-1 "
                                        ...
                                        {{ __('levels.add') }}</a>
                                    <a href="{{ route('parcel.create_multiple') }}" class="btn btn-sm btn-primary "
                                        data-toggle="tooltip" data-placement="top" title="multiple"><i class="fa fa-plus"></i>
                                        {{ __('parcel.add_multiple') }}</a>
                                    <a href="{{ route('parcel.multiple.print-label-perday') }}" class="btn btn-sm btn-primary "
                                        data-toggle="tooltip" data-placement="top" title="multiple"></i>Per Day</a>
                                </div>
                            </div>
                        @endif
                    </div>
                    <div class="card-body">
                        <div class="table-responsive">
                            <table id="table" class="table    parcelTable" style="width:100%">
                                ...
                                <tbody>
                                    @php
                                        $counter = ($parcels->currentPage() - 1) * $parcels->perPage() + 1;
                                    @endphp
                                            ...
                                            <td>
                                                <div class="row">
                                                    ...
                                                    <div class="dropdown-menu">
                                                        ...
                                                        <a href="{{ route('parcel.print-label', $parcel->id) }}" target="_blank"
                                                            class="dropdown-item"><i class="fas fa-print"
                                                        ...
                                                        @if(\App\Enums\ParcelStatus::DELIVERED !== $parcel->status && \App\Enums\ParcelStatus::PARTIAL_DELIVERED !== $parcel->status)
                                                            @if(hasPermission('parcel_update') == true)
                                                                <a href="{{route('parcel.edit', $parcel->id)}}" class="dropdown-item"><i
                                                                        class="fas fa-edit" aria-hidden="true"></i>
                                                                    {{__('levels.edit')}}</a>
                                                            @endif
                                                            @if(hasPermission('parcel_delete'))
                                                                <form id="delete" value="Test"
                                                                    action="{{route('parcel.delete', $parcel->id)}}" method="POST"
                                            ...
                                            <td>
                                                <div class="w250">
                                                    ...
                                                    @if ($parcel->return_to_courier == App\Enums\BooleanStatus::YES)
                                                        {{__('levels.return_charges')}}: <span
                                                            class="text-dark">{{settings()->currency}}{{$parcel->return_charges}}</span>
                                                        <br>
                                                    @else
